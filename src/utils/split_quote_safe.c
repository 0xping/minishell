...
static size_t	count_chunks_non_empty(char *cmd, char *dels) {
  ...
  result = 0;
  while (cmd[i]) {
    if ((i == 0 && !ft_strchr(dels, cmd[i])) ||
        (ft_strchr(dels, cmd[i]) && !ft_strchr(dels, cmd[i + 1]) &&
         cmd[i + 1] && !is_char_quoted(cmd, &cmd[i])))
      result++;
    i++;
  }
  return (result);
  ...
  while (i < words) {
    if (i == 0 && ft_strchr(dels, s[0]))
      ...
      if (s[start] && ft_strchr(dels, s[start]) &&
          !is_char_quoted(s, &s[start]))
        start++;
      end = start;
      while (s[end] && (!ft_strchr(dels, s[end]) || is_char_quoted(s, &s[end])))
        end++;
...
static void	fill_arr(size_t words, char *s, char *dels, char ***result) {
  ...
  start = 0;
  while (i < words) {
    while (s[start] && ft_strchr(dels, s[start]) &&
           !is_char_quoted(s, &s[start]))
      start++;
    end = start;
    while (s[end] && (!ft_strchr(dels, s[end]) || is_char_quoted(s, &s[end])))
