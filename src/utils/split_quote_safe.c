...
/*   Updated: 2023/06/22 17:19:38 by aait-lfd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static size_t	count_chunks(char *cmd, char *dels, bool handle_empty_str)
{
	...
	{
		if (handle_empty_str)
			cond = ft_strchr(dels, cmd[i]) && !is_char_quoted(cmd, &cmd[i]);
		else
...
static void	fill_arr(size_t words, char *s, char *dels, char ***result)
{
	...
	while (i < words)
	{
		while (ft_strchr(dels, s[start]) && s[start])
			start++;
		end = start;
		while (!ft_strchr(dels, s[end]) && s[end])
			end++;
		(*result)[i] = ft_substr(s, start, end - start);
		start = end;
		i++;
